...
function Page() {
  ...
  const fetchBookings = async (status) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/equipment/getAllBookings?status=${status}`
      );
  ...
  const handleApprove = async (bookingId) => {
    try {
      await axios.put(
        `http://localhost:5000/api/equipment/approve/${bookingId}`
      );
      setBookings(
  ...
  const handleReject = async (bookingId) => {
    try {
      await axios.put(
        `http://localhost:5000/api/equipment/reject/${bookingId}`
      );
      setBookings(
        bookings.map((booking) =>
          booking._id === bookingId
          ...
          <table className="table table-xs w-full">
            <thead className="bg-gray-200 sticky top-0 z-10">
              <tr>
                ...
                <th className="w-1/4 px-4 py-2 text-left">Status</th>
